...
class Sql
{
  ...
  // ONLINE  
    ...
    public function selectTablaNew($tabla,$filtro,$filtro_por,$orden,$limit){
     ...
     public function marcos($id){
         $this->connect();
         $sql = "SELECT marco,tamano FROM pedidos WHERE id >= $id";
         //echo $sql;
         $result = $this->connection->query($sql);
            while($row = $result->fetch_assoc()) {
                    $array['marco'] = $row['marco'];
                    $array['tamano'] = $row['tamano'];
                    $this->select[] = $array;
    ...
    public function sumas($tabla,$columna,$pago){
            $this->connect();
            $sql = "SELECT SUM($columna) as total FROM $tabla WHERE pago = '$pago'";
            //echo $sql;
            $result = $this->connection->query($sql);
                while($row = $result->fetch_assoc()) {
                    echo $row['total'];
     ...
     public function suma_total($tabla,$columna){
            $this->connect();
            $sql_2 = "UPDATE $tabla SET $columna = REPLACE($columna, '$', '')";
            $this->connection->query($sql_2);
